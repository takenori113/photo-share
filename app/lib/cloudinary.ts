...
export async function uploadImage(
  ...
  return new Promise<{ url: string; public_id: string }>((resolve, reject) => {
    // 元ファイルのMIMEタイプを保持し、Cloudinary側で正しく解釈させる
...
export async function findImagesByPrefix(prefix: string) {
  return new Promise<{ resources: Array<{ url: string; public_id: string }> }>(
    (resolve, reject) => {
      // 複数の検索条件を組み合わせて検索
      ...
      cloudinary.search
        ...
        .then((result) => {
          console.log(
            "Cloudinary search result count:",
          ...
          if (!result.resources || result.resources.length === 0) {
            console.log(
              "No results found with filename search, trying with public_id",
            );
            ...
            cloudinary.search
              ...
              .then((fallbackResult) => {
                console.log(
                  ...
                  fallbackResult.resources?.length || 0,
                );
